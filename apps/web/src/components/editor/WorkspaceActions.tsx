...
import type { ResolvedKeybindingsConfig, EditorId } from "@t3tools/contracts";

const EMPTY_KEYBINDINGS: ResolvedKeybindingsConfig = { rules: [] };
 ...
 */
function projectIdFromScopeKey(scopeKey: string): string | null {
  if (scopeKey.startsWith("project:")) {
    return scopeKey.slice("project:".length);
  }
  return null;
}
...
export function WorkspaceActions() {
  ...
  const activeProject = scopeProjectId
  ...
  const routeThreadId = useParams({
    strict: false,
    select: (params) =>
      params.threadId ? ThreadIdSchema.makeUnsafe(params.threadId) : null,
  });
  ...
  const keybindings =
    serverConfigQuery.data?.keybindings ?? EMPTY_KEYBINDINGS;
  ...
  const storeSetTerminalOpen = useTerminalStateStore(
    (s) => s.setTerminalOpen,
  );
