...
function createGitHubCliWithFakeGh(scenario: FakeGhScenario = {}): {
  ...
  const pullRequestQueue = [...(scenario.pullRequestSequence ?? [])];
  ...
  const execute: GitHubCliShape["execute"] = (input) => {
    ...
    if (args[0] === "pr" && args[1] === "view") {
      const pullRequest: FakePullRequest = pullRequestQueue.shift() ??
    ...
    if (args[0] === "pr" && args[1] === "merge") {
      const reference = args[2];
      if (typeof reference === "string") {
        const failures = scenario.mergePullRequestFailures?.[reference];
...
function mergePullRequests(
  ...
  return manager.mergePullRequests(input);
}
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  ...
  it.effect("tolerates retarget failures when GitHub already updated the stacked PR base", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      const remoteDir = yield* createBareRemote();
      ...
      const { manager, ghCalls } = yield* makeManager({
        ghScenario: {
          ...
          editPullRequestBaseFailures: {
            "102": new GitHubCliError({
              operation: "execute",
              detail: "GraphQL: Base branch was already changed",
            }),
          },
        },
      });
  ...
  it.effect(
    "retries a stacked PR merge when GitHub is still recalculating after retargeting",
    () =>
      Effect.gen(function* () {
        const repoDir = yield* makeTempDir("t3code-git-manager-");
        yield* initRepo(repoDir);
        ...
        const { manager, ghCalls } = yield* makeManager({
          ghScenario: {
            ...
            mergePullRequestFailures: {
              "102": [
                new GitHubCliError({
                  operation: "execute",
                  detail: "Pull request is not mergeable yet",
                }),
              ],
            },
          },
        ...
        expect(result.merged.map((pullRequest) => pullRequest.number)).toEqual([101, 102]);
        expect(ghCalls.filter((call) => call === "pr merge 102 --squash")).toHaveLength(2);
      }),
    12_000,
  );

  it.effect(
    "keeps retrying the final stacked PR merge while GitHub recomputes mergeability",
    () =>
      Effect.gen(function* () {
        const repoDir = yield* makeTempDir("t3code-git-manager-");
        yield* initRepo(repoDir);
          ...
          ghScenario: {
            ...
            mergePullRequestFailures: {
              "102": [
                ...
                new GitHubCliError({
                  operation: "execute",
                  detail: "Base branch was modified. Review and try the merge again.",
                }),
                new GitHubCliError({
                  operation: "execute",
                  detail: "Pull request is not mergeable yet",
        ...
        expect(result.merged.map((pullRequest) => pullRequest.number)).toEqual([101, 102]);
        expect(ghCalls.filter((call) => call === "pr merge 102 --squash")).toHaveLength(5);
      }),
    20_000,
  );
    ...
    Effect.gen(function* () {
      ...
      const { manager, ghCalls } = yield* makeManager({
        ghScenario: {
          ...
          onMergePullRequest: ({ reference }) => {
            if (reference === "101") {
              runGitSyncForFakeGh(repoDir, ["push", "origin", "--delete", "feature/stack-root"]);
            }
          },
        },
      });
  ...
  it.effect("cleans up already merged branches when a later PR in the stack fails", () =>
    Effect.gen(function* () {
        ...
        ghScenario: {
          ...
          mergePullRequestFailures: {
            "102": [
              new GitHubCliError({
                operation: "execute",
                detail: "Unexpected merge failure",
              }),
            ],
          },
          onMergePullRequest: ({ reference }) => {
      ...
      const errorMessage = yield* mergePullRequests(manager, {
        cwd: repoDir,
        scope: "stack",
        method: "squash",
        deleteBranch: true,
      ...
      expect(errorMessage).toContain("Failed to merge PR #102.");
      expect(
        yield* runGit(repoDir, ["branch", "--list", "feature/stack-root"]).pipe(
          Effect.map((result) => result.stdout.trim()),
        ),
      ...
      expect(
        yield* runGit(repoDir, ["branch", "--list", "feature/stack-tip"]).pipe(
          Effect.map((result) => result.stdout.trim()),
        ),
      ).toContain("feature/stack-tip");
    }),
  );
});
